          ...
          {banners?.results?.length > 0 &&
            banners.results.map((item) => (
              <SwiperSlide key={item.id}>
                <BannerSlide name={item.title} src={`https://image.tmdb.org/t/p/original${item.backdrop_path}`} tags={item.genre_ids} id={item.id}></BannerSlide>
              </SwiperSlide>
            ))}
        </Swiper>
      </div>
